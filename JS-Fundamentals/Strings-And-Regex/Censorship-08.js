function solve(text, stringToBeCensored) {

    for(let i = 0; i < stringToBeCensored.length; i++){
        text = text.replace(new RegExp(stringToBeCensored[i], 'g'), '-'.repeat(stringToBeCensored[i].length));
    }
